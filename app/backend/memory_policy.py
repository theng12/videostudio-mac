...
# "performance" pins a loaded pipeline in unified memory forever. That is a
# fine default for an app that owns its machine — and a bad one for the
# deployment that actually exists, where 3-5 Studios share a single 8 GB Mac
# and none of them knows the others are resident. Each Studio ships this same
# skeleton with this same default, so every one of them independently
# concluded that holding its pipeline forever was free.
#
...
# job at all. The idle-release thread was running the whole time — it just
# had nothing to do, because idle_seconds is None in this mode.
#
